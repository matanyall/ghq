...
ghq.worktreeMode::
    When set to true, repositories are cloned into a subdirectory named after the default branch
    (e.g., +~/ghq/github.com/user/repo/main+). This makes it easier to create multiple worktrees
    for the same repository under different branch directories. +
    This setting can also be controlled by the 'GHQ_WORKTREE_MODE' environment variable. +
    Defaults to false. This option is ignored for bare repositories.
...
GHQ_WORKTREE_MODE::
    If set to any non-empty value, enables worktree mode. When enabled,
    repositories are cloned into a subdirectory named after the default branch.
    This overrides the 'ghq.worktreeMode' configuration setting.
...
When 'ghq.worktreeMode' is enabled, repositories include the default branch name:
